...
import { useLaunchpadInfo, useReactiveContractCall } from "@/hooks/useContractCall";
	...
	<div class="w-full grid gap-4">
		...
		>
			<div class="grid grid-cols-2 gap-2 mb-2">
				...
				<h3 class="text-xl font-extralight ml-4" v-if="info.currentPhase == 0">
					{{ $t("Prebond") }}
				</h3>
			</div>
			...
			<div v-show="!slippage" class="flex flex-col">
				...
				>
					<template #lead>
						<div v-if="useTokenInput" class="flex items-center gap-2 ml-2">
							<img
				...
				>
					...
					<BaseButton @click="tokens = info.balance" size="xss" secondary>
						100%
					</BaseButton>
					<BaseButton @click="tokens = info.balance * 0.1" size="xss" secondary>
						10%
					</BaseButton>
					<BaseButton
					...
					<BaseButton @click="tokens = info.balance * 0.5" size="xss" secondary>
						50%
					</BaseButton>
					<BaseButton
				...
				<div
					...
					class="flex items-center gap-1 mt-2 bg-gray-800 p-1 rounded-lg"
				>
					<BaseButton @click="amount = 0" size="xss" outlined secondary>
						{{ $t("RESET") }}
					</BaseButton>
			...
			<div v-show="slippage" class="flex flex-col">
				<FormInput v-model="slippagePercent" size="sm" input-classes="!pl-24">
					<template #lead>
				...
				<div class="flex items-center gap-1 mt-2 bg-gray-800 p-1 rounded-lg">
					<BaseButton
			...
			<div v-if="authCheck && isConnected" class="flex flex-col gap-2">
				<TxStatus
					v-if="
						['sellTokens', 'buyTokens', 'contributePreBonding'].includes(
							state.called
						)
					"
					class="mb-2"
		...
		>
			...
			<TxStatus v-if="state.called === 'withdrawTokenAllocation'" :state="state" />
