...
defineEmits(["update:modelValue", "changed", "focused", "unfocused", "onKeyUp"]);
...
<template>
	<div>
		...
		<div v-if="$slots.trail || $slots.lead" class="relative rounded shadow-sm">
			<div
				v-if="$slots.lead"
			...
			<input
				...
				:class="[
					...
					error
						? 'bg-red-50 border-red-500  text-red-900 placeholder-red-700 rounded focus:ring-red-500 focus:border-red-500  dark:bg-red-100 dark:border-red-400'
						: 'bg-white border-gray-300 text-gray-900  rounded focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-900 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white',
					classes,
					...(Array.isArray(inputClasses) ? inputClasses : [inputClasses]),
				]"
		...
		<p v-if="error" class="mt-2 text-sm !text-red-600 dark:!text-red-400">
			<template v-if="typeof error === 'string' || error instanceof String">
				{{ error }}
			</template>
